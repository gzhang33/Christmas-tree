...
export const DebugStore: React.FC = () => {
    ...
    const isLowFps = useStore((state) => state.isLowFps);
            ...
            {isExploded && (
                <div className="mt-4 pt-3 border-t border-white/10 space-y-2">
                    ...
                    <div className="flex justify-between">
                        ...
                        <span className={isLowFps ? 'text-red-400 font-bold' : 'text-green-400'}>
                            {currentFps !== null ? `${currentFps}` : 'N/A'}
                        </span>
                    </div>
                    {isLowFps && (
                        <div className="text-xs text-red-400 bg-red-400/10 px-2 py-1 rounded">
                            ⚠️ Low FPS detected (&lt;30fps)
                        </div>
                    )}
                </div>
            )}
