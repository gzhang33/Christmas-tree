...
export const Controls: React.FC<ControlsProps> = ({ uiState }) => {
  ...
  return (
    <div className="absolute top-0 right-0 h-full pointer-events-none flex flex-col items-end z-10 p-4">
      ...
      {/* Main Panel */}
      <div
        className={`pointer-events-auto bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl p-6 w-80 text-white shadow-2xl transition-transform duration-300 ease-in-out transform origin-top-right overflow-y-auto max-h-[90vh] ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none hidden'
          }`}
      >
